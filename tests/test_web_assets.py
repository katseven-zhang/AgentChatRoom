...
def test_web_declares_a_static_favicon():
    ...
    # #158/#159：彻底消除绿油油标签图标，采用 Apple System Blue 经典双色微光渐变。
    assert "#0f766e" not in favicon
    assert "url(#brand-grad)" in favicon
    assert "#0077ed" in favicon
    assert "#005bb5" in favicon
...
def test_web_management_grid_pagination_and_workspace_drawer():
    ...
    drawer_start = markup.index("<details")
    assert "高级诊断：Host 与 Workspace 登记" in markup
...
def test_web_modern_tech_indigo_palette_and_smart_refresh():
    """#157/#159：告别绿油油与高饱和紫色视疲劳，重构为舒适护眼、极致典雅的 macOS HIG 与 OpenAI Codex 配色体系（Apple System Blue & System Gray）。"""
    stylesheet = (WEB_DIR / "app.css").read_text(encoding="utf-8")
    javascript = (WEB_DIR / "app.js").read_text(encoding="utf-8")
    markup = (WEB_DIR / "index.html").read_text(encoding="utf-8")

    # 1. 彻底告别 #176b57 森林绿与高饱和刺眼紫色，升级为 Apple System Blue & System Gray
    assert "#176b57" not in stylesheet
    assert "#4338ca" not in stylesheet
    assert "--bg: #f5f5f7;" in stylesheet
    assert "--primary: #0066cc;" in stylesheet
    assert "--primary-soft: #edf5fd;" in stylesheet
    assert "--accent-fill: #0071e3;" in stylesheet

    # 2. 暗色主题采用 macOS Dark 护眼灰与 Apple System Blue
    assert "--bg: #1c1c1e;" in stylesheet
    assert "--surface: #2c2c2e;" in stylesheet
    assert "--primary: #70baff;" in stylesheet
    assert "--accent-fill: #0071e3;" in stylesheet
...
def test_web_supports_human_reading_and_guided_interactions():
    ...
    # #149：composer 高级选项折叠区整体移除，发送区只保留输入框与发送按钮。
    assert "composer-advanced" not in markup
    assert "composer-options" not in markup
...
def test_web_typography_baseline_wraps_long_content_and_narrow_selects():
    ...
    # #149：composer 下拉控件随高级选项一起移除，窄屏不再有横向溢出源。
    assert ".composer-options select {" not in stylesheet


def test_web_composer_advanced_options_carry_real_semantics_and_hints():
    """Regression for tasks #54→#149: the composer now carries ONLY the
    input and send button; submission pins the generic protocol defaults
    (kind/channel/task/priority/ack) instead of exposing controls."""
    markup = (WEB_DIR / "index.html").read_text(encoding="utf-8")
    javascript = (WEB_DIR / "app.js").read_text(encoding="utf-8")
    # 高级控件全部移除。
    for fragment in (
        'id="message-kind"', 'id="message-channel"', 'id="message-task"',
        'id="message-priority"', 'id="message-requires-ack"',
        'id="composer-advanced"', "高级选项",
    ):
        assert fragment not in markup
    # 提交固定使用通用协议参数，与后端 MESSAGE_KINDS/CHANNELS 语义一致。
    submit = javascript[javascript.index('elements["message-form"].addEventListener'):]
    for pair in ('kind: "message"', 'channel: "public"', "task_id: null", "priority: 2", "requires_ack: false"):
        assert pair in submit
...
def test_web_recent_activity_switch_projects_without_crosstalk(tmp_path):
    ...
    harness.write_text(
        ...
        "assert.equal(elements['recent-event-list'].innerHTML.includes('Agent A'), false);\n"
        "assert.equal(elements['recent-event-list'].innerHTML.includes('B-event-2'), true);\n"
        "assert.equal(elements['recent-event-list'].innerHTML.includes('<strong>Agent B</strong> task.created'), true);\n",
        encoding="utf-8",
    )
    run = subprocess.run(["node", str(harness)], capture_output=True, text=True)
    assert run.returncode == 0, run.stderr
...
def test_web_recent_activity_headings_carry_agent_names_and_paging(tmp_path):
    ...
    harness.write_text(
        ...
        "// created_at deliberately out of id order: id 4 is the OLDEST event\n"
        "state.events = [\n"
        "  {id: 1, project_seq: 11, event_type: 'task.claimed', actor_session_id: 's-opencode', payload: {title: 'T1'}, created_at: '2026-09-15T10:04:00Z'},\n"
        "  {id: 2, project_seq: 12, event_type: 'message.message', actor_session_id: 's-codex', payload: {body: 'hello'}, created_at: '2026-09-15T10:03:00Z'},\n"
        "  {id: 3, project_seq: 13, event_type: 'lease.acquired', actor_session_id: '', payload: {path_pattern: 'p'}, created_at: '2026-09-15T10:02:00Z'},\n"
        "  {id: 4, project_seq: 14, event_type: 'message.message', actor_session_id: 's-gone', payload: {body: 'hi'}, created_at: '2026-09-15T10:01:00Z'},\n"
        "];\n"
        "renderRecent();\n"
        "let html = elements['recent-event-list'].innerHTML;\n"
        "assert.equal(html.includes('<strong>OpenCode</strong> task.claimed'), true);\n"
        ...
        "assert.equal(html.includes('undefined'), false);\n"
        "// strict created_at desc: newest timestamp first even when ids disagree\n"
        "assert.ok(html.indexOf('#11') < html.indexOf('#12'));\n"
        "assert.ok(html.indexOf('#12') < html.indexOf('#13'));\n"
        "assert.ok(html.indexOf('#13') < html.indexOf('#14'));\n"
