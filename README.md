...
提交兜底（`agentchatroom submit`）：MCP 传输不可用（transport 会话被回收、连接器假死且无法立即重载）时，同一项目凭据经运行中服务的 REST API 仍必定能完成提交。`agentchatroom submit --project <名称或id> (--bundle <acrb.v1 文件或字面量> | --token <acr.*>) (--message <正文> | --report-task <任务号或id> --summary <摘要> [--file ...] [--test 命令::退出码 ...] [--commit ...] | --no-code-change <原因>)` 一次性完成：按凭据定位 Project 并服务端校验（吊销/过期凭据返回 `submit_credential_invalid` 与补发凭据的 required_action）→ 自动注册工作区（等价 room_bootstrap 的 register_workspace，报告不再出现裸 409）→ 恰好加入一次会话 → 需要时按同身份 reclaim 接管旧会话任务 → 提交消息/报告 → 退出。路径、端口与凭据全部来自参数与配置（`--url` 与全局配置一致），进程一次性不驻留。MCP 可用时仍以 MCP 工具为常规路径。
