...
管理 Tab 只保留纯项目级资产（项目文档、成员、Token、Workspace、审计与归档），全局运维自 #146 起收拢到顶栏「系统设置」弹窗。成员、Token 与审计历史采用单列卡片列表展示，严格按每页 5 行分页呈现，提供翻页控件；审计末行不足 5 行时以空占位槽补齐，翻页不跳动；Host 与 Workspace 登记折叠为「高级诊断」抽屉。服务启动维护时会自动清理已过期与已吊销的失效 Token（仍被会话引用的除外），保持 Token 列表整洁。
