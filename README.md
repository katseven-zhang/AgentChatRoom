...
- **双击 / 无参数 / `gui`**：轻量控制台（默认形态）。顶部控制条提供监听 IP 输入框、端口输入框、「启动服务」「重启服务」「停止服务」「打开前端」按钮与实时状态指示（●运行中 含地址与 pid / ○已停止）。IP 仅接受合法 IP 地址（如 `127.0.0.1`、`0.0.0.0`、`::`），端口仅接受 1-65535 整数；启动成功后地址与端口写回本地配置，下次启动直接生效。非环回绑定（如 `0.0.0.0` 或局域网 IP）按服务端安全规则要求配置已开启管理认证，否则拒绝启动并提示原因。启动复用分离后台服务生命周期（pid 文件、健康检查、进程树清理）；「重启服务」一键先停止后启动；「打开前端」调用系统默认浏览器访问当前运行地址。日志区实时滚动显示服务日志，令牌、密钥等敏感值自动脱敏，并有行数上限防止内存膨胀。
...
- **日志区**：滚动显示启动、运行错误与停止日志，日志中的令牌、密钥等敏感值会被遮蔽，不回显明文，并有行数上限防止内存膨胀。
