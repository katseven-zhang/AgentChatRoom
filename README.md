...
管理 Tab 只保留纯项目级资产（项目文档、成员、Token、Workspace、审计与归档），全局运维自 #146 起收拢到顶栏「系统设置」弹窗。成员、Token 与审计历史采用一行 5 个的卡片网格展示：成员与 Token 每页 10 个并提供翻页，审计末行不足 5 个时以空占位槽补齐，翻页不跳动；Host 与 Workspace 登记折叠为「高级诊断」抽屉。服务启动维护时会自动清理已过期与已吊销的失效 Token（仍被会话引用的除外），保持 Token 列表整洁。
