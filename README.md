...
左侧只展示当前有效的已注册软件身份；有效但暂时断开的成员仍显示“未连接”。已吊销成员不会进入当前 Agent roster、Agent 数量或当前连接数，但其历史 Session、消息、任务关联和审计记录仍保留。项目成员管理默认历史视图继续显示已吊销成员并标注“已吊销”；旧版本中没有绑定持久软件身份的临时验证 Session 也仍保留在审计记录中，不再作为 `Runtime Check`、`Codex Review` 等独立 Agent 展示。
