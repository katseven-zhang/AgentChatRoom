...
- For an assigned task, inspect it with `task_get` or `task_get_by_number`, then use `task_acknowledge`. Use `task_claim` for an unowned eligible task. To resume unfinished work from a disconnected Session of the same software identity, call `task_claim(reclaim=true)` explicitly; reclaim is rejected while the owner is connected. If the old Session cannot be restored but is still considered connected, and the user has asked this conversation to continue that specific task, call `task_claim(takeover=true)` explicitly; it transfers owner and associated leases atomically and revokes the old Session's task/lease write authority. Neither form is automatic or available across software identities. Before editing shared files, acquire an appropriate `lease_acquire` path lease; release it with `lease_release` when the work or handoff ends.
...
- Resuming work is explicit: prefer `room_bootstrap` restoration of the original Session. With a newly created Session, `task_claim(reclaim=true)` succeeds after the same-identity owner is treated as disconnected; if it is still connected, `task_owner_session_connected` points to `task_claim(takeover=true)` as an explicit, audited alternative, including for ghost heartbeats. Only use live takeover when the user has directed this conversation to continue that task: it preempts a genuinely active parallel Session too. A different software identity or Project is always rejected (`task_reclaim_forbidden`). Never bypass ownership with `task_update(status=todo)`, fabricated IDs, or another Session's token.
- Except for the separately authorized `task_claim(takeover=true)` path, only the owner's own credentials or an authenticated management request may release, reassign, or rewrite a task. A missing Session credential is not management authority: `task_update` / `task_release` / `task_assign` without owner credentials and without management authorization are rejected (`management_auth_required`). Never copy another Session's token, guess an identity, or use a management-only entry point to take over work.
